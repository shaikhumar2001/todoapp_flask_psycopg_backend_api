...
def login(request=request):
    ...
    if not email or not password:
        return response_template(success=False, error_code=400, message="email and password required", status=400, request=request)

    rows = db.execute_query("SELECT * FROM todoapp.tusertbl WHERE email=%s", (email,))
    if not rows:
        return response_template(success=False, error_code=401, message="Invalid credentials", status=401, request=request)
    user = rows[0]
    ...
    return response_template(success=True, error_code=0, message="Logged in", data={"access_token": access}, request=request)
